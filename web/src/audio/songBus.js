...
export const songBus = {
  ...
  strikeTimes: null, // [[t…] × 5] per-finger sorted strike times, in SECONDS
  getTime: () => 0, // live playback position in seconds (reads the audio clock)
};

/**
 * Attach a compiled song. `events` / `strikeTimes` come from compileSong().
 * `getTime` must return the current playback position in seconds.
 */
export function setSongSource(events, strikeTimes, getTime) {
  songBus.events = events;
  songBus.strikeTimes = strikeTimes;
